...
from backend.atomic_writer import atomic_write_json, atomic_write_text
...
def get_core_token() -> str:
    ...
    atomic_write_text(CORE_TOKEN_FILE, token)
    return token
...
def generate_contextproxy_core_config() -> dict:
    ...
    config["config_version"] = str(int(time.time() * 1000))
    config["config_hash"] = _config_hash(config)
    atomic_write_json(CORE_CONFIG_FILE, config, backup=True)
    write_log(
        "core",
        (
            "generated contextproxy core config: "
            f"path={CORE_CONFIG_FILE} domain_rules={len(config['domain_rules'])} "
            f"process_rules={len(config['process_rules'])} listeners={len(group_ports)} "
            f"config_version={config['config_version']} config_hash={config['config_hash']}"
        ),
    )
    return config
