...
def generate_all_configs():
    """Generate mihomo/config.yaml and report whether the runtime config changed.

    The returned mapping is intentionally backward-compatible for callers that
    only need a truthy result, while giving apply_mihomo_config_change() enough
    information to avoid restarting mihomo when the generated config is
    identical to the currently loaded file.
    """
    ...
    backup_file = MAIN_CONFIG_FILE.with_name(MAIN_CONFIG_FILE.name + ".bak")
    result = {
        "files": {"config.yaml"},
        "changed": False,
        "config_path": str(MAIN_CONFIG_FILE),
        "backup_path": str(backup_file),
        "config_hash": current_hash or "",
    }
    ...
    try:
        ...
        if MAIN_CONFIG_FILE.exists():
            atomic_write_bytes(backup_file, MAIN_CONFIG_FILE.read_bytes())
