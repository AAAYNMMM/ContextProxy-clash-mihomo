...
func (c *Core) handleHealth(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{
		...
		"loaded_config_version": cfg.ConfigVersion,
		"loaded_config_hash":    cfg.ConfigHash,
	})
}
...
func (c *Core) handleMetrics(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{
		"ok":                                            true,
		"loaded_config_version":                         cfgForIdle.ConfigVersion,
		"loaded_config_hash":                            cfgForIdle.ConfigHash,
		"active":                                        activeCount,
...
func (c *Core) handleReload(w http.ResponseWriter, r *http.Request) {
	...
	c.clearReports()
	cfg := c.getConfig()
	c.logger.Printf("config reloaded version=%s hash=%s domain_rules=%d process_rules=%d listeners=%d", cfg.ConfigVersion, cfg.ConfigHash, len(cfg.DomainRules), len(cfg.ProcessRules), len(cfg.Groups))
	c.addEvent(CoreEvent{Source: "core", SourceReason: "config_reloaded", Action: "reload"})
	_ = json.NewEncoder(w).Encode(map[string]any{
		"ok":                    true,
		"loaded_config_version": cfg.ConfigVersion,
		"loaded_config_hash":    cfg.ConfigHash,
		"domain_rule_count":     len(cfg.DomainRules),
		"process_rule_count":    len(cfg.ProcessRules),
		"listener_count":        len(cfg.Groups),
	})
}
