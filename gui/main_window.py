...
from gui.settings_store import load_app_settings
...
from backend.config_apply import (
    save_app_settings_and_apply,
    save_domain_rules_and_apply,
    save_group_nodes_and_apply,
    save_process_rules_and_apply,
)
...
class MainWindow(QMainWindow):
    ...
    def _create_group_from_dialog(self):
        ...
        def task():
            save_group_nodes_and_apply(config_to_save, allow_running=False, reason=f"group_create:{group_name}")
    ...
    def _delete_current_group(self):
        ...
        def task():
            save_group_nodes_and_apply(config_to_save, allow_running=False, reason=f"group_delete:{group_name}")
            return config_to_save
    ...
    def _save_domain_rules(self):
        ...
        def task():
            result = save_domain_rules_and_apply(rules, old_rules=old_rules)
            return result.changed_groups, result.changed_items
        ...
        def on_success(result):
            ...
            if is_proxy_running():
                self._append_activity_log(
                    f"[INFO] 域名规则已保存并已由 Go core reload 生效，"
                    f"groups={sorted(changed_groups)}, patterns={sorted(changed_patterns)}"
                )
                self.notify_success("域名规则已保存并生效")
            else:
                self._append_activity_log("[INFO] 域名规则已保存，启动代理后生效")
    ...
    def _save_process_rules(self):
        ...
        def task():
            result = save_process_rules_and_apply(rules, old_rules=old_rules)
            return result.changed_groups, result.changed_items
        ...
        def on_success(result):
            ...
            if is_proxy_running():
                self._append_activity_log(
                    f"[INFO] 进程规则已保存并已由 Go core reload 生效，"
                    f"groups={sorted(changed_groups)}, processes={sorted(changed_patterns)}"
                )
                self.notify_success("进程规则已保存并生效")
            else:
                self._append_activity_log("[INFO] 进程规则已保存，启动代理后生效")
    ...
    def _save_app_settings_from_inputs(self):
        ...
        def task():
            save_app_settings_and_apply(settings, previous_settings=previous_settings)
            return message
