...
from backend.atomic_writer import atomic_write_text
...
def _save_rule_file_internal(path: Path, rules: list[tuple[str, str]]) -> None:
    """Internal file-only helper. Prefer backend.config_apply for save-and-apply."""
    text = "".join(f"{group},{value}\n" for group, value in rules)
...
def save_domain_rules_file_only(rules: list[tuple[str, str]]) -> None:
    """File-only save helper. Runtime apply must use backend.config_apply."""
    _save_rule_file_internal(DOMAIN_RULES_FILE, rules)
...
def save_process_rules_file_only(rules: list[tuple[str, str]]) -> None:
    """File-only save helper. Runtime apply must use backend.config_apply."""
    _save_rule_file_internal(PROCESS_RULES_FILE, rules)
